...
Baseline = ["Event","LapTime", "NextLapTime","LapNumber", "PLapTime", "PPLapTime", "PPPLapTime"]
...
for j in range(len(years)):
    ...
    for k in range(len(events)):
        ...
        for i in range(3,len(laps)-1):
            if ((laps.loc[i+1,"DriverNumber"] == laps.loc[i,"DriverNumber"]) & (laps.loc[i-3,"DriverNumber"] == laps.loc[i,"DriverNumber"]) ):

                #baseline 
                LapTime = timeToMs(laps.loc[i,"LapTime"])
                PLapTime = timeToMs(laps.loc[i-1,"LapTime"])
                PPLapTime = timeToMs(laps.loc[i-2,"LapTime"])
                PPPLapTime = timeToMs(laps.loc[i-3,"LapTime"])
                ...
                raceData.loc[index] = ({"Event":Event,"LapTime":LapTime,"PLapTime":PLapTime,"PPLapTime":PPLapTime,"PPPLapTime":PPPLapTime
                                        ,"NextLapTime":NextLapTime, "LapNumber": LapNumber, "DriverNumber":DriverNumber,"Team": Team, "Compound":Compound,
...
raceData.to_csv("./raceData2019MultiplePrevLaps.csv")
